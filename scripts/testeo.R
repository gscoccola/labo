...
#hiperparametros del mejor modelo (en train)
PARAM$finalmodel$max_bin           <-     31
PARAM$finalmodel$learning_rate     <-      0.0437907293266036   
PARAM$finalmodel$num_iterations    <-    164  
PARAM$finalmodel$num_leaves        <-   1024  
PARAM$finalmodel$min_data_in_leaf  <-   1239  
PARAM$finalmodel$feature_fraction  <-      0.324462828627375  
...
p1 <- ggplot(resultados, aes(x=envios, y=ganancias)) + geom_line()+ ggtitle(toString(c("default", PARAM$finalmodel$semilla, ganancia_aprox)))+ geom_point(
  aes(y = ganancias), size=1.5, color="red") +  geom_hline(yintercept=ganancia_aprox, color="blue", size=0.3)
